...
`SoloRun` remains a configurable progression model. Fresh Adventure parties use five bouts: a level-3 initiation followed by fights at levels 5, 8, 10, and 12, with reward targets 5, 8, 10, 12, and a final level-12 cache. Parties loaded at level 5, 8, 10, or 12 reconstruct only the remaining suffix. Victory over the level-12 Astral Exarch fixture crowns the champion.

`SoloArena` connects that model to the engine. It treats the active one-to-four-character player party as one cooperative team, so the same flow supports solo, online co-op, and local split-screen. Every wave matches the active player count, capped at three initiation enemies or four tournament enemies. Levels 3 and 5 run two waves; levels 8 and 10 run three; level 12 runs four rotating Exarch waves. `Bg3Adapter` ensures every player and temporary opponent has BG3's native `DeathSavingThrows` passive. At zero HP, the actor remains downed in initiative and receives temporary hidden protection against repeated AI hits; healing or a natural recovery removes that protection.

Clearing a non-final wave leaves combat, clears temporary relations, deletes that wave's enemies, and restores only downed players to partial health. It intentionally does not refresh surviving characters, spell slots, class resources, or cooldowns before the next wave spawns three seconds later. Clearing the final wave performs common cleanup and a full long-rest-equivalent restoration, then generates the deterministic reward bundle. For this playable alpha, every player receives four native `RewardMedium` rolls and all six rare candidates are distributed round-robin across party inventories. Non-final victories award the vanilla experience delta and wait for native level-up completion. The level-12 reward completes the run and schedules three party-wide championship announcements. Defeats and draws in any wave fully restore the party and rebuild the same tier from wave 1 after five seconds.
...
`ArenaLayouts` contains twelve deterministic formations. The runtime selects a formation from the run, bout, and wave IDs, so an identical run cannot reroll positioning by reissuing a command. `ArenaSites` maps levels 3 and 5 to Astral Flats, level 8 to Crescent Ruin, and levels 10 and 12 to Echelon Steps. `Bg3Adapter` records the initial staging origin, teleports each party member with explicit offsets and AI-grid ground snapping, and returns the party during common final cleanup.
