...
`ArenaLayouts` contains twelve deterministic formations. The runtime selects a formation from the run, bout, and wave IDs, so an identical run cannot reroll positioning by reissuing a command. `ArenaSites` maps levels 3 and 5 to Astral Flats, level 8 to Crescent Ruin, and level 10 to Echelon Steps. `Bg3Adapter` records the initial staging origin, teleports each party member with explicit offsets and AI-grid ground snapping, and returns the party during common final cleanup.
