...
After a victory, the runtime generates a deterministic six-choice offer at the next tier. The host cycles through items and party recipients using BG3's native controller-safe Yes/No dialog; no console command is required. The chosen recipient receives two native `RewardMedium` treasure rolls and one selected item; then every player-party member receives the vanilla experience delta for the next target level. Defeat and draw cleanup use the same dialog to offer an immediate replay. BG3's ordinary level-up screen remains authoritative for all class, subclass, spell, feat, and multiclass decisions. The next bout cannot begin until all active party members report the expected level.
